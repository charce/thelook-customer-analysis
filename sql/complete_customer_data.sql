...
-- grain: user_id × order_id (one row per customer-order; customers with no orders included)
-- sources: bigquery-public-data.thelook_ecommerce.users, bigquery-public-data.thelook_ecommerce.orders
-- notes: Includes all customers; most_recent_order_* derived per user (may be NULL if no orders)

CREATE OR REPLACE TABLE `merch-funnel.customer_analysis.complete_customer_data` AS
WITH user_orders AS (
  SELECT
    u.id AS user_id,
    u.first_name,
